...
class Demo:
    ...
    def LineInterp(self, frames):
        ...
        for i in range(total_line):
            ...
            interp_buf_start_x[i, :] = np.round(np.linspace(
                start1_x, start2_x, frames)).astype(np.int32)
            interp_buf_start_y[i, :] = np.round(np.linspace(
                start1_y, start2_y, frames)).astype(np.int32)
            interp_buf_end_x[i, :] = np.round(np.linspace(
                end1_x, end2_x, frames)).astype(np.int32)
            interp_buf_end_y[i, :] = np.round(np.linspace(
                end1_y, end2_y, frames)).astype(np.int32)
        ...
        self.warpline = [[np.zeros([total_line, 2]), np.zeros(
            [total_line, 2])] for x in range(frames)]
    ...
    def Morphing(self, frames):
        ...
        for frame in range(1, frames):
            ...
            for line in range(total_line):
                cos_theta = np.dot(vec_map_source[line], vec_map_dest[
                                   line]) / (len_map_dest[line] * len_map_source[line])
